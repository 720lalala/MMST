    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(action.compareTo("-")==0)
        {
            ...
            else
            {
                if(sales1.equals(slaes2))
                {
                    response.getWriter().print( "<script>alert(\"请确认输入的衣服id是否有库存!\");window.location.href='/allCanAccept/sales.jsp'</script>");
                }
                else
                {
                    response.getWriter().print( "<script>alert(\"请确认输入的衣服id是否有库存!\");window.location.href='/admin/allSalesmain.jsp'</script>");
                }
            }
