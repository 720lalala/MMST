...
public class insertDepotServlet extends HttpServlet {
            ...
            throws IOException, ServletException
    {
       ...
       // System.out.println(clothingid);
        String numberss=request.getParameter("numberssI");
        ...
        if(bool){
            ...
            if(bool)
            {
                ...
                response.getWriter().print( "<script>alert(\"请确认输入的衣服id是否存在!\");window.location.href='/allCanAccept/depotmain.jsp'</script>");
                 }

        }
        else{
            //System.out.println("??");
            response.getWriter().print( "<script>alert(\"请确认输入的店铺id是否存在!\");window.location.href='/allCanAccept/depotmain.jsp'</script>");
         }
